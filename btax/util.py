...
def read_from_egg(tfile):
    ...
    template_path = os.path.join(path, tfile)
...
def get_paths():
    ...
    paths['_FIN_ACCT_DIR'] = _FIN_ACCT_DIR = os.path.join(_RAW_DIR,
                                                          'national_accounts')
    paths['_OUT_DIR'] = os.environ.get('BTAX_OUT_DIR', 'btax_output_dir')
    if not os.path.exists(paths['_OUT_DIR']):
        os.mkdir(paths['_OUT_DIR'])
    paths['_BEA_ASSET_PATH'] = _os.path.join(_BEA_DIR,
                                             "detailnonres_stk1.xlsx")
    paths['_SOI_BEA_CROSS'] = _os.path.join(_BEA_DIR,
    ...
    paths['_ECON_DEPR_IN_PATH'] = os.path.join(_DEPR_DIR,
                                               'Economic Depreciation Rates.csv')
    paths['_TAX_DEPR'] = os.path.join(_DEPR_DIR, 'tax_depreciation_rates.csv')
    paths['_SOI_DIR'] = os.path.join(_RAW_DIR, 'soi')
    paths['_CORP_DIR'] = os.path.join(_SOI_DIR, 'soi_corporate')
    paths['_TOT_CORP_IN_PATH'] = os.path.join(_CORP_DIR, '2013sb1.csv')
    paths['_S_CORP_IN_PATH'] = os.path.join(_CORP_DIR, '2013sb3.csv')
    paths['_PRT_DIR'] = os.path.join(_SOI_DIR, 'soi_partner')
    paths['_DETAIL_PART_CROSS_PATH'] = os.path.join(_PRT_DIR,
                                    'partner_crosswalk_detailed_industries.csv')
    ...
    paths['_PROP_DIR'] = os.path.join(_SOI_DIR, 'soi_proprietorship')
    paths['_PRT_DIR'] = os.path.join(_SOI_DIR, 'soi_partner')
    ...
    paths['_DETAIL_SOLE_PROP_CROSS_PATH'] = os.path.join(_PROP_DIR,
                                            'detail_sole_prop_crosswalk.csv')
