...
def calibrate_financing():
    ...
    num_rows = [1, 4]
    # reads the equity data from the .csv file. Specifies which columns and
    # rows to read in the file
    kw = dict(skiprows=skipped[5], usecols=columns,
              header=None, names=column_name, nrows=num_rows[1])
    corp_equity_df = pd.read_csv(_EQUITY_CORP, **kw)

    where_zero = corp_equity_df.index == 0
    amt = corp_equity_df[where_zero]['Amount'][0]
    non_fin_corp_equity = amt * _CST_FACTOR
    # apportions the equity based on the ratio of
    # equity held by each industry
    non_fin_data = {'non_fin_corp_equity': non_fin_corp_equity}
    equity_values = apportion_equity(non_fin_data)
    df = pd.read_csv(_DEBT_NFCORP,
                     skiprows=skipped[0],
                     usecols=columns,
                     header=None,
                     names=column_name,
                     nrows=num_rows[0])
    non_fin_corp_debt = df['Amount'][0] * _CST_FACTOR
    ...
    eq_3 = corp_equity_df.index == 3
    amt = corp_equity_df[eq_3]['Amount']
    fin_corp_equity = amt[3] * _CST_FACTOR
    eq = apportion_equity({'fin_corp_equity': fin_corp_equity})
    equity_values.update(eq)
    df = pd.read_csv(_DEBT_FCORP, skiprows=skipped[2], usecols=columns,
                     header=None, names=column_name, nrows=num_rows[0])
    fin_corp_debt = df['Amount'][0] * _CST_FACTOR
    debt_values.update(apportion_debt({'fin_corp_debt': fin_corp_debt}))

    non_corp_equity = pd.read_csv(_EQUITY_NCORP, skiprows=skipped[4],
                                  usecols=columns, header=None,
                                  names=column_name,
                                  nrows=num_rows[0])['Amount'][0] * _CST_FACTOR
    eq = apportion_equity({'non_corp_equity': non_corp_equity})
    equity_values.update(eq)
    df = pd.read_csv(_DEBT_NCORP, skiprows=skipped[1], usecols=columns,
                     header=None, names=column_name, nrows=num_rows[0])
    non_corp_debt = df['Amount'][0] * _CST_FACTOR
    debt_values.update(apportion_debt({'non_corp_debt': non_corp_debt}))

    # calculates the overall debt ratios based on the equity and debt values,
    # stored in dictionaries {key: dataframe}
    debt_params = calc_debt(equity_values, debt_values)
    return debt_params


def apportion_debt(total_liab):
    # use the ratio of total liabilities in an industry to
    # total interest paid by all industries to proportionally distribute debt
    ...
    # runs the debt calculation twice for s-corps and c-corps or
...
def apportion_equity(total_equity):
    ...
    if((keyword == 'non_fin_corp_equity') or (keyword == 'fin_corp_equity')):
        ...
        for i in types:
            ...
            equity[i]['cost_of_treasury_stock'] = st_1
...
def calc_debt(total_equity, total_liab):
    # Calculates the debt ratio for each industry and
    # then uses it to calculate the real discount rate
    ...
    equity = total_equity['non_corp_equity'].sum(axis=1)
    non_corp_equity = equity + s_corp_equity
    ...
    df = pd.read_csv(_NAICS_CODES)
    dfs = [df, debt_f_corp, debt_f_non_corp]
    total_debt_f = pd.concat(dfs, axis=1)
...
def calc_after_return(indust_debt):
    ...
    ret = equity_ratio * real_rate_return
    rate = nominal_mrkt_intrst - inflation_rate
    after_tax_return = debt_ratio * rate + ret


def save_ratios(debt_ratios):
    debt_ratios = debt_ratios[(debt_ratios.NAICS == '11') |
                              (debt_ratios.NAICS == '211') |
                              (debt_ratios.NAICS == '212') |
                              (debt_ratios.NAICS == '213') |
                              (debt_ratios.NAICS == '22') |
                              (debt_ratios.NAICS == '23') |
                              (debt_ratios.NAICS == '31-33') |
                              (debt_ratios.NAICS == '32411') |
                              (debt_ratios.NAICS == '336') |
                              (debt_ratios.NAICS == '3391') |
                              (debt_ratios.NAICS == '42') |
                              (debt_ratios.NAICS == '44-45') |
                              (debt_ratios.NAICS == '48-49') |
                              (debt_ratios.NAICS == '51') |
                              (debt_ratios.NAICS == '52') |
                              (debt_ratios.NAICS == '531') |
                              (debt_ratios.NAICS == '532') |
                              (debt_ratios.NAICS == '533') |
                              (debt_ratios.NAICS == '54') |
                              (debt_ratios.NAICS == '55') |
                              (debt_ratios.NAICS == '56') |
                              (debt_ratios.NAICS == '61') |
                              (debt_ratios.NAICS == '62') |
                              (debt_ratios.NAICS == '71') |
