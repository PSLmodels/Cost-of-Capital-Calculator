...
sole_str = 'Sole proprietorships and partnerships'
sole = bea_residential['entity_type'] == sole_str
...
cmp_str = ('Households; owner-occupied real estate '
           'including vacant land and mobile homes at market value')
owner_occ_house_RE = np.array(b101.ix[b101['Variable'] == cmp_str, 'Value'])
...
total_btax_FA = total_btax_assets.copy()
total_btax_FA -= total_btax_LAND
total_btax_FA -= total_btax_INV
total_btax_FA -= total_btax_RES_FA
...
'''
Do differences in fixed assets by industry
(the place where BEA data has detail)
'''
...
gr = btax_FA.groupby(['bea_ind_code'])
summ = gr['assets_c', 'assets_nc', 'assets'].sum()
shares_corp_non = pd.DataFrame(summ).reset_index()
assets_c = shares_corp_non['assets_c']
assets_nc = shares_corp_non['assets_nc']
shares_corp_non['Corp Share'] = assets_c / (assets_c + assets_nc)
shares_corp_non['Non-corp Share'] = assets_nc / (assets_c + assets_nc)
assets_c = shares_corp_non['assets_c']
assets_nc = shares_corp_non['assets_nc']
assets = shares_corp_non['assets']
shares_corp_non['check'] = (assets_c + assets_nc) - assets
...
assets_c = part_corp_non['assets_c']
assets_nc = part_corp_non['assets_nc']
part_corp_non['Corp Share'] = assets_c / (assets_c + assets_nc)
assets_nc = part_corp_non['assets_nc']
assets_c = part_corp_non['assets_c']
part_corp_non['Non-corp Share'] = assets_nc / (assets_c + assets_nc)
assets_c = part_corp_non['assets_c']
assets_nc = part_corp_non['assets_nc']
assets = part_corp_non['assets']
part_corp_non['check'] = (assets_c + assets_nc) - assets
# merge in industry names
df3 = asset_data[['Industry', 'bea_ind_code']].copy()
df3.drop_duplicates(inplace=True)
