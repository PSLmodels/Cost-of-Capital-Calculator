...
-------------------------------------------------------------------------------
Module that handles reading in the soi partner data.
Contains one long script that loads in the capital stock,
income and entity information. Using the different asset and
income information (ratios of assets to income),
...
def load_partner_data(entity_dfs):
    """Reads in the partner data and creates new dataframes for each
       partner type and stores them in the soi dictionary
    ...
    2) read in detailed xwalk to go from string names for industries to
       SOI codes
    3) In xwalk, keep only "complete = 1" codes
    3) do an inner join of the two to keep only unique codes
    4) summ over the ind code from crosswalk- because some repeated,
       those rep more detailed classifcations summing to larger SOI category
    5) Now have partner assets at as detailed industry codes as we can get
    6) merge these partner codes to the soi_bea_ind_codes xwalk to get the
       sector, major, minor industry codes
         - need to do 3 merges, 1 for those id's at two digit,
           1 for those at 3 and one for those at 6
    7) Take corp data and summ assets by major ind to create ratios
       for each of 195 inds
       id'd in corp data - ratio of assets for that ind as a share of
       all in that major ind

    8) merge these ratios to part data, will join on partner ind code to
       indy_cd from corp data and will have partner
       assets repeat for minor ind
    9) multiply ratios by assets - unless part already at minor ind -
       should have what want now could do at major industry
    - do have 811 at minor industry
    """
    # Opening data on depreciable fixed assets, inventories,
    ...
    deprec_assets = df03['Depreciable assets']
    less_accum = df03['Less:  Accumulated depreciation']
    ...
    sector_df = pd.merge(df03_sector, soi_bea_ind_codes,
                         how='inner', left_on=['INDY_CD'],
                         right_on=['sector_code'], left_index=False,
                         right_index=False, sort=False,
                         copy=True, indicator=True)
    major_df = pd.merge(df03_major, soi_bea_ind_codes,
                        how='inner', left_on=['INDY_CD'],
                        right_on=['major_code'], left_index=False,
                        right_index=False, sort=False,
                        copy=True, indicator=True)
    minor_df = pd.merge(df03_minor, soi_bea_ind_codes,
                        how='inner', left_on=['INDY_CD'],
    ...
    corp = pd.merge(s_corp, soi_bea_ind_codes,
    ...
    # industry splits aren't adding to SOI control total
    ...
    part_types = {
        'Corporate general partners': 'corp_gen',
    ...
    # reshape data
    df05 = pd.melt(df05, id_vars=['Item'], value_vars=[
        'Corporate general partners',
        'Corporate limited partners',
        'Individual general partners',
        'Individual limited partners',
    ...
    df05 = pd.merge(df05, grouped,
                    how='left', left_on=['Codes:'],
    ...
    for key in part_types:
        df05.loc[(df05['Codes:'] == 31) &
                 (df05['part_type'] == key), 'inc_ratio'] = \
            df05.loc[(df05['Codes:'] == 1) &
    ...
    sector_df = pd.merge(df05_sector, soi_bea_ind_codes,
                         how='inner', left_on=['Codes:'],
                         right_on=['sector_code'], left_index=False,
                         right_index=False, sort=False, copy=True,
                         indicator=True)
    major_df = pd.merge(df05_major, soi_bea_ind_codes,
                        how='inner', left_on=['Codes:'],
                        right_on=['major_code'], left_index=False,
                        right_index=False, sort=False,
                        copy=True, indicator=True)
    minor_df = pd.merge(df05_minor, soi_bea_ind_codes,
                        how='inner', left_on=['Codes:'],
                        right_on=['minor_code'], left_index=False,
                        right_index=False, sort=False,
                        copy=True, indicator=True)
    ...
    part_assets = pd.merge(df05, part_data,
    ...
    inc_ratio = part_assets['inc_ratio']
    fixed = part_assets['Fixed Assets']
    part_assets['Fixed Assets'] = fixed * inc_ratio
    inv = part_assets['Inventories']
    part_assets['Inventories'] = inv * inc_ratio
    land = part_assets['Land']
    part_assets['Land'] = land * inc_ratio
    depr = part_assets['Depreciation']
