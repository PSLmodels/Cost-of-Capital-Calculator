...
-------------------------------------------------------------------------------
Module that handles reading in the soi corporate data.
Contains a script that loads in the capital stock
information for total corporations and s corporations.
Based on the ratio of assets for total corporations,
data for s corporations is imputed. Finally, using the difference
between the two, the c corporation
...
def load_corp_data():
    ...
    tot_corp = pd.merge(tot_corp, soi_bea_ind_codes,
    ...
    # for "all industries". It's within 1%, but difference
    # can't be accounted for (sum over industry > totals for all industries)

    s_corp.rename(columns={"LAND": "Land",
                           "INVNTRY": "Inventories",
                           "DPRCBL_ASSTS": "Fixed Assets",
                           "NET_DPR": "Depreciation",
                           "INDY_CD": "minor_code_alt"},
                  inplace=True)
...
def calc_proportions(tot_corp, s_corp, columns):
    """Uses the ratio of the minor industry to the major industry to
    fill in missing s corp data.
        ...
        :type columns: list
        :returns: capital stock for the s corporations with all the
    ...
    # New data w just ratios that will then merge to s corp data
    # by sector code (many to one merge)
