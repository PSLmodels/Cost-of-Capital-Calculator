...
def init_depr_rates(asset_tree):
    ...
    econ_depr = calc_rates.calc_depr_rates(fixed_asset_tree, inv_tree, land_tree)
    tax_depr = calc_rates.calc_tax_depr_rates(fixed_asset_tree, inv_tree, land_tree)
    depr_rates = pd.DataFrame(index = np.arange(0,len(fixed_asset_tree.enum_inds)), columns = ('NAICS', 'Econ_Corp', 'Econ_Non_Corp', 'Tax_Corp', 'Tax_Non_Corp'))
    depr_rates['NAICS'] = econ_depr['NAICS']
    depr_rates['Econ_Corp'] = econ_depr['Corp']
