...
setattr(
    DepreciationParams,
    "defaults",
    os.path.join(CURRENT_PATH, "..", "tax_depreciation_rules.json"),
)
...
@pytest.mark.parametrize(
    "include_land,include_inventories",
...
def test_range_plot(corporate):
    ...
    assert fig
    fig = calc.range_plot(
        calc2, output_variable="rho", corporate=corporate, include_title=True
    )
    assert fig
...
def test_grouped_bar(corporate):
    ...
    assert fig
    fig = calc.grouped_bar(
        calc2, output_variable="rho", corporate=corporate, group_by_asset=False
    )
    assert fig
...
def test_p_param_return_value():
    ...
    assert np.allclose(obj, np.array([0.31331849]))
