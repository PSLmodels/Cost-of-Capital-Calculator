...
def pull_soi_data():
    ...
    c_corp = entity_dfs['c_corp'][['minor_code_alt', 'Land',
                                   'Fixed Assets', 'Inventories']].copy()
    c_corp.loc[:, 'entity_type'] = 'c_corp'
    s_corp = entity_dfs['s_corp'][['minor_code_alt', 'Land',
                                   'Fixed Assets', 'Inventories']].copy()
    s_corp.loc[:, 'entity_type'] = 's_corp'
    partner = entity_dfs['part_data'][['minor_code_alt', 'Land',
                                       'Fixed Assets', 'Inventories',
                                       'part_type']].copy()
    partner.loc[:, 'entity_type'] = 'partnership'
    sole_prop = entity_dfs['sole_prop_data'][['minor_code_alt', 'Land',
                                              'Fixed Assets',
                                              'Inventories']].copy()
