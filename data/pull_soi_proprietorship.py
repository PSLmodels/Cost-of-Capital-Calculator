...
def load_proprietorship_data(entity_dfs):
    ...
    # Keep only variables of interest
    nonfarm_inv = nonfarm_inv[['Net income status, item',
                               'Inventory, end of year']]
    nonfarm_inv['Net income status, item'] =\
        nonfarm_inv['Net income status, item'].str.strip()
    ...
    part_data = entity_dfs['part_data'][['minor_code_alt', 'part_type']
                                        + columns +
                                        ['Land', 'Fixed Assets']].copy()
    ...
    part_ratios['FA_ratio'] = (part_ratios['Fixed Assets'] /
                               part_ratios['Depreciation'])
    ...
    bea_ratio = 1.
    nonfarm['Fixed Assets'] = (nonfarm['FA_ratio'] *
                               nonfarm['Depreciation'] * bea_ratio)
