...
def load_partner_data(entity_dfs):
    ...
    df = format_excel(pd.read_excel(_AST_FILE, skiprows=2, skipfooter=6))
    ...
    df03["Item"] = df03["Item"].apply(lambda x: re.sub(r"[\s+]", "", x))

    # partner data - income
    df01 = format_excel(pd.read_excel(_INC_FILE, skiprows=2, skipfooter=6))
    # Cuts off the repeated columns so only the data for all
    # partnerships remains
    ...
    df01["Item"] = df01["Item"].apply(lambda x: re.sub(r"[\s+]", "", x))
    ...
    for var in columns:
        corp[var + "_ratio"] = corp.groupby(["major_code"])[var].apply(
    ...
    grouped = pd.DataFrame(
        {"sum": df05.groupby(["Codes:"]).apply(abs_sum, "net_inc")}
    ).reset_index()
    ...
    df05["inc_ratio"] = (
        df05["net_inc"].astype(float).abs() / df05["sum"].replace({0: np.nan})
    ).replace({np.nan: 0})
    ...
    part_assets = df05.merge(
        part_data, how="left", on=["minor_code_alt"], copy=True, indicator=True
    )
