...
def get_paths():
    ...
    paths["_DEPR_DIR"] = _DEPR_DIR = os.path.join(
        _DATA_DIR, "depreciation_rates"
    )
    paths["_BEA_DIR"] = _BEA_DIR = os.path.join(_RAW_DIR, "BEA")
    paths["_FIN_ACCT_DIR"] = _FIN_ACCT_DIR = os.path.join(
        _RAW_DIR, "national_accounts"
    )
    paths["_BEA_ASSET_PATH"] = os.path.join(_BEA_DIR, "detailnonres_stk1.xlsx")
    paths["_SOI_BEA_CROSS"] = os.path.join(
        _BEA_DIR, "soi_bea_industry_codes.csv"
    )
    paths["_BEA_INV"] = os.path.join(_BEA_DIR, "NIPA_5.8.5B.xls")
    paths["_BEA_RES"] = os.path.join(
        _BEA_DIR, "BEA_StdFixedAsset_Table5.1.xls"
    )
    ...
    paths["_TYP_IN_CROSS_PATH"] = os.path.join(
        _PRT_DIR, "13pa05_Crosswalk.csv"
    )
    paths["_TYP_FILE"] = os.path.join(_PRT_DIR, "13pa05.xls")
    paths["_PROP_DIR"] = _PROP_DIR = os.path.join(
        _SOI_DIR, "soi_proprietorship"
    )
    paths["_PRT_DIR"] = _PRT_DIR = os.path.join(_SOI_DIR, "soi_partner")
