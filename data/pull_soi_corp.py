...
def load_corp_data():
    ...
    try:
        ...
        s_corp = s_corp.drop(s_corp[s_corp["AC"] > 1.0].index)
        # drop total across all industries
        s_corp = s_corp.drop(s_corp[s_corp["INDY_CD"] == 1.0].index)
    ...
    try:
        ...
        tot_corp = tot_corp.drop(tot_corp[tot_corp["AC"] > 1.0].index)
        # drop total across all industries
        tot_corp = tot_corp.drop(tot_corp[tot_corp["INDY_CD"] == 1.0].index)
    ...
    c_corp = tot_corp.merge(
        s_corp, how="inner", on=["INDY_CD"], suffixes=("_x", "_y"), copy=True
    )
